...
function showOverlay(text) {
  ...
  _overlay.style.cssText = `position:fixed;inset:0;z-index:2147483647;background:rgba(0,0,0,0.78);backdrop-filter:blur(4px);display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;pointer-events:all;cursor:not-allowed;`;
  ...
  lbl.style.cssText = `color:#e2e8f0;font-size:15px;font-weight:700;font-family:-apple-system,BlinkMacSystemFont,sans-serif;text-align:center;max-width:340px;line-height:1.55;`;
  lbl.textContent = text;
  const sub = document.createElement('div');
  sub.style.cssText = `color:rgba(255,255,255,.38);font-size:12px;font-family:-apple-system,BlinkMacSystemFont,sans-serif;text-align:center;animation:owui-pulse 2s ease-in-out infinite;`;
...
function isPerplexityUIText(t) {
  ...
  return (/^reviewed?\s+\d+\s+sources?\.?$/i.test(t) || /^\d+\s+sources?\.?$/i.test(t) ||
    /^searching(\.\.\.)?$/i.test(t) || /^generating(\.\.\.)?$/i.test(t) ||
    /^thinking(\.\.\.)?$/i.test(t) || /^sources?:?$/i.test(t) ||
    /^related$/i.test(t) || /^answer$/i.test(t) || /^pro search$/i.test(t) ||
    /^focus:/i.test(t) || /^\d+\s+steps?\s+completed/i.test(t) ||
    /^(🚀|⬇|⏳|✅|❌)/.test(t) || t.length < 4);
}
